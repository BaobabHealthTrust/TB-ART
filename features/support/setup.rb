...
Before do
  ...
  `mysql --user=openmrs --password=openmrs #{database} < #{File.join(RAILS_ROOT, 'db', 'openmrs_metadata.sql')}`
  `mysql --user=openmrs --password=openmrs #{database} < #{File.join(RAILS_ROOT, 'db', 'data', 'nno', 'nno.sql')}`
