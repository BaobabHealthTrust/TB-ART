...
class SurvivalAnalysis
  ...
  def self.report(cohort)
    ...
    ( date_ranges || [] ).each_with_index do | range ,i | 
      ...
      survival_analysis_outcomes["#{(i)*12} month survival: outcomes by end of #{survival_end_date.strftime('%B %Y')}"] = {'Number Alive and on ART' => 0, 
      ...
      (states || [] ).each do | patient_id , state |
        case state
          when 'PATIENT TRANSFERRED OUT'
             survival_analysis_outcomes["#{(i)*12} month survival: outcomes by end of #{survival_end_date.strftime('%B %Y')}"]['Number Transferred out']+=1 
          when 'PATIENT DIED'
             survival_analysis_outcomes["#{(i)*12} month survival: outcomes by end of #{survival_end_date.strftime('%B %Y')}"]['Number Dead']+=1 
          when 'TREATMENT STOPPED'
             survival_analysis_outcomes["#{(i)*12} month survival: outcomes by end of #{survival_end_date.strftime('%B %Y')}"]['Number Stopped Treatment']+=1 
          when 'ON ANTIRETROVIRALS'
             survival_analysis_outcomes["#{(i)*12} month survival: outcomes by end of #{survival_end_date.strftime('%B %Y')}"]['Number Alive and on ART']+=1 
        end
      end
    end
